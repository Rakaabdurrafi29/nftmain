...
export default function Turnover() {
    const { getMyNetworkTurnover } = useMembers();
    const { auth } = usePage().props;
    const [myNetworkTurnover, setMyNetworkTurnover] = useState(null);

    useEffect(() => {
        const fetchData = async () => {
            setMyNetworkTurnover(await getMyNetworkTurnover(auth.user.id));
        };
