...
export default function MyTurnover() {
    const { getMyTurnover } = useMembers();
    const { auth } = usePage().props;
    const [myTurnover, setMyTurnover] = useState(null);

    useEffect(() => {
        const fetchData = async () => {
            setMyTurnover(await getMyTurnover(auth.user.id));
        };
