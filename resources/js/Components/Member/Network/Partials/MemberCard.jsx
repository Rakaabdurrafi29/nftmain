export default function MemberCard({
    ...
    if (rank_id === 1) text = "text-stone-400";
    if (rank_id === 2) text = "text-amber-700";
    if (rank_id === 3) text = "text-zinc-400";
    if (rank_id === 4) text = "text-amber-400";
    if (rank_id === 5) text = "text-lime-400";
    if (rank_id === 6) text = "text-cyan-400";
    if (rank_id === 7) text = "text-indigo-400";

    if (rank_id === 1) bg = "bg-stone-100";
    if (rank_id === 2) bg = "bg-amber-700";
    if (rank_id === 3) bg = "bg-zinc-400";
    if (rank_id === 4) bg = "bg-amber-400";
    if (rank_id === 5) bg = "bg-lime-400";
    if (rank_id === 6) bg = "bg-cyan-400";
    if (rank_id === 7) bg = "bg-indigo-400";

    console.log(auth);
    return (
        <div className="col-span-1 sm:col-span-5 md:col-span-3 lg:col-span-3 xl:col-span-2">
                ...
                <div className="card-body relative">
                    <a href="#">
                        <div className="flex">
                            ...
                            />
                            <span
                                className={`absolute left-2 h-3 w-3 rounded-full ${bg} p-2 flex justify-center items-center border-2 border-white`}
                            ></span>

                            <div className="flex-shrink self-center ml-2 w-9/12">
                                <h6 className="w-full font-medium text-base text-slate-300 truncate mb-1">
                                    {username}
                                </h6>
                                <span
                                    className={`text-xs ${text} font-medium flex`}
                                >
                                    {is_leader === 1 && (
                                        <svg
                                            width="18"
